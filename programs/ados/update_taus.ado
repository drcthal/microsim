program define update_taus
	...
	`vol' {		
		...
		replace tau_c = min(tau_c,1/24) if (`day' - day_infected) == days_to_symptomatic & runiform()<=`pr_isolate'
		
		// return to work if recovered
		replace tau_w = base_tau_w if recovered
		replace tau_c = base_tau_c if recovered
